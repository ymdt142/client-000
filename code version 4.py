...
import docx
with open("coordinate.txt",'r') as obj:
    ...
    for i in content:
        ...
        X.append(i[1])
        y.append(i[2])
df=pd.DataFrame({"Element":Element,"X":X,"y":y})
...
for index, row in df.iterrows():
    ...
    if name not in dont_change:
        ...
        if (not checkMax(belongsTo, closest, user_y, maximum)):
            new_ele[old] = user_element + " " + str(closest) + " " + str(user_y)
            old+=1
        else:
        ...
        """if index == df.shape[0] - 1:
            ...
            try:
                for index, row in op3.iterrows():
                    user_element = row["Element"]
                    user_x = int(row["X"])
                    true_user_Y = int(row["y"])
                with open("FinalOutput.txt.", 'a+') as obj:
                    obj.write(user_element + " " + user_x + " " + true_user_Y + "\n")
        ...
        new_ele[old] = user_element+" "+str(user_x)+" "+str(true_user_Y)
        old += 1
        print("Dont Change name  ",name)


try:
    for k,v in new_ele.items():
        """user_element = row["Element"]
        user_x = int(row["X"])
        true_user_Y = int(row["y"])"""
        print(k,v)
        with open("FinalOutput.txt.", 'a+') as obj:
            obj.write(str(v)+"\n")
    geeky_file = open("counterFinal.txt", 'wt')
    geeky_file.write(str(counter))
    geeky_file.close()
